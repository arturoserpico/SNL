...
namespace snl {
	...
	public:
		...
		template<typename... SymDependencies> 
			requires (sizeof...(SymDependencies) == sizeof...(Dependencies))
		Sym(
			std::function<T(Dependencies...)> fun, 
			std::tuple<SymDependencies...> dependencies
		) : fun(fun) {
			allocDep(dependencies);
		}
		...
		Sym(
			std::function<T(Dependencies...)> fun,
			const SymDependencies&... dependencies
		) : Sym(fun, std::tuple(dependencies...)) {}
