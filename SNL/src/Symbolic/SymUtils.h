...
namespace snl {
	...
	auto convertArgsToSymRef(T first, auto&&... rest) {
		Sym<std::conditional_t<std::is_same_v<TargetList, TypeList<>>, T, SafeGet<TargetList, index>>> result;

		if constexpr (std::is_same_v<TargetList, TypeList<>>)
			result = Sym<T>(first);
		else
			result = static_cast<Get<TargetList, index>>(first);
	...
	auto convertArgsToSymRef(const Sym<T>& first, auto&&... rest) {
		...
		if constexpr (std::is_same_v<TargetList, TypeList<>>)
			...
			result = Sym(first).cast<Get<TargetList, index>>();
